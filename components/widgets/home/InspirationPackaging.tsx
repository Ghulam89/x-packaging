...
const InspirationPackaging = () => {
    ...
    const images = [
        { src: ASSETS.gallery.g09, alt: imageDescriptions[0] },
        { src: ASSETS.gallery.g08, alt: imageDescriptions[1] },
        { src: ASSETS.gallery.g05, alt: imageDescriptions[2] },
        { src: ASSETS.gallery.g10, alt: imageDescriptions[3] },
        { src: ASSETS.gallery.g04, alt: imageDescriptions[4] },
        { src: ASSETS.gallery.g03, alt: imageDescriptions[5] },
        { src: ASSETS.gallery.g02, alt: imageDescriptions[6] },
        { src: ASSETS.gallery.g07, alt: imageDescriptions[7] },
        { src: ASSETS.gallery.g06, alt: imageDescriptions[8] },
        { src: ASSETS.gallery.g11, alt: imageDescriptions[9] },
        { src: ASSETS.gallery.g12, alt: imageDescriptions[10] },
        { src: ASSETS.gallery.g13, alt: imageDescriptions[11] },
        { src: ASSETS.gallery.g14, alt: imageDescriptions[12] },
        { src: ASSETS.gallery.g15, alt: imageDescriptions[13] },
        { src: ASSETS.gallery.g16, alt: imageDescriptions[14] }  
    ];
    ...
    const openImageViewer = useCallback((index: number) => {
        setSelectedImage(images[index].src as any);
    ...
    const goToPrevious = useCallback(() => {
        ...
        setSelectedImage(images[newIndex].src as any);
    ...
    useEffect(() => {
        if (swiperRef.current) {
            ...
            const timer = setTimeout(() => {
                if (swiper && typeof (swiper as any).update === 'function') {
(swiper as any).update();
(swiper as any).updateSlides();
                    (swiper as any).updateSlidesClasses();
                }
                const middleIndex = Math.floor(images.length / 2);
                if (swiper && typeof (swiper as any).slideTo === 'function') {
                    (swiper as any).slideTo(middleIndex, 0);
                }
                if ((swiper as any).autoplay && typeof (swiper as any).autoplay.start === 'function') {
        ...
        <div className="mx-auto overflow-x-hidden py-10 bg-[#f7f7f7]">
            ...
            <div className='w-full mx-auto pb-8 pt-3 overflow-hidden'>
              ...
              <Swiper
                ...
                onSwiper={(swiper) => {
                  (swiperRef as React.MutableRefObject<any>).current = swiper;
                          ...
                          onLoad={() => {
                            ...
                            if (swiperRef.current) {
                              setTimeout(() => {
                                ...
                                if (s && Array.isArray((s as any).slides) && (s as any).slides.length > 0) {
                                  (s as any).update();
                                  (s as any).updateSlides();
                                  (s as any).updateSlidesClasses();
                                }
                              }, 50);
                            }
                          }}
                    ...
                    <div className="absolute bottom-20 left-1/2 -translate-x-1/2 flex gap-2 max-w-4xl overflow-x-auto px-4 py-2 bg-white/5 backdrop-blur-md rounded-full border border-white/10 custom-scrollbar">
                        {images.map((img, idx) => (
                            <button
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    setSelectedImage(img.src as any);
                                    setCurrentIndex(idx);
                                }}
