...
const CreativeGallery = ({ products = [], title, description }) => {
  ...
  // Convert products to images format
      ...
      <div className="max-w-8xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-16">
          ...
          <div className="md:col-span-8 group relative overflow-hidden rounded-[2.5rem] bg-white p-4 shadow-xl shadow-slate-200/50 border border-lime-100">
            {images.find(img => img.url === activeImage)?.slug ? (
              <Link to={`/product/${images.find(img => img.url === activeImage).slug}`}>
                <div className="overflow-hidden rounded-[2rem] h-[500px] cursor-pointer">
                  <img
                    src={activeImage}
                    alt={images.find(img => img.url === activeImage)?.title || "Featured Packaging"}
          ...
          <div className="md:col-span-4 flex flex-col gap-6">
            {images[1] && (
              images[1].slug ? (
                <Link to={`/product/${images[1].slug}`}>
                  <div className="h-1/2 group relative overflow-hidden rounded-[2rem] bg-white p-3 shadow-lg border border-lime-100 cursor-pointer">
            ...
            <div className="h-1/2 grid grid-cols-2 gap-4">
              {images[2] && (
                images[2].slug ? (
                  <Link to={`/product/${images[2].slug}`}>
                    <div className="rounded-[1.5rem] overflow-hidden border-2 border-lime-200 p-1 bg-white cursor-pointer">
