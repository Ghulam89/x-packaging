...
const ProducedCard = ({ data }) => {
  return (
    <div className="bg-transparent rounded-md p-2 sm:p-4">
      <div className="grid sm:grid-cols-3 grid-cols-1 gap-4 sm:gap-6 items-stretch">
        {data.map((item, index) => (
          <div
            key={item.title}
            className="col-span-1 bg-white border border-gray-200 rounded-xl sm:rounded-2xl p-5 sm:p-6 min-h-[220px] sm:min-h-[240px] hover:-translate-y-2 hover:shadow-xl transform transition-all duration-300 ease-in-out group relative overflow-hidden"
          >
            ...
            <div className="relative z-10 flex flex-col h-full pt-8 sm:pt-10">
              ...
              {/* Title */}
              <h3 className="text-lg sm:text-xl font-bold text-[#213554] mb-3 sm:mb-4 text-center sm:text-left group-hover:text-[#EE334B] transition-colors duration-300 leading-tight">
                {item?.title}
              </h3>

              {/* Description */}
              <p className="text-sm sm:text-base text-gray-600 leading-relaxed flex-grow text-center sm:text-left group-hover:text-gray-700 transition-colors duration-300">
                {item?.desc}
              </p>
            </div>
