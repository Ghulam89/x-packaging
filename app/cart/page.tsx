                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                    <div className="flex-shrink-0">
                      <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-xl overflow-hidden bg-gradient-to-br from-[#213554]/5 to-[#EE334B]/5 border border-gray-100">
                        {item.image ? (
                          <img
                            src={`${siteOrigin}/${String(item.image).replace(/^\//, "")}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-4 mb-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-lg font-semibold text-[#EE334B]">${toNumber(item.price).toFixed(2)}</p>
                        </div>
                        <button
                      ...
                      <div className="flex items-center gap-4 mt-4">
                        ...
                        <div className="flex items-center gap-2 border border-gray-300 rounded-lg overflow-hidden">
                          <button
                            onClick={() =>
                              handleQuantityChange(item._id, Math.max(1, toNumber(item.quantity) - 1))
                            }
                          ...
                          <input
                            type="number"
                            min={1}
                            value={Math.max(1, toNumber(item.quantity))}
                            onChange={(e) => handleQuantityChange(item._id, toNumber(e.target.value))}
                            className="w-16 h-10 text-center border-0 focus:ring-2 focus:ring-[#EE334B] text-[#213554] font-semibold"
                          />
                          <button
                            onClick={() => handleQuantityChange(item._id, Math.max(1, toNumber(item.quantity) + 1))}
