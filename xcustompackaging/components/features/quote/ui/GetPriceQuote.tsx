...
const GetPriceQuote = () => {
  ...
  const [submitMsg, setSubmitMsg] = useState<{ type: "success" | "error" | null; message: string }>({ type: null, message: "" });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    if (!validateForm()) {
      setSubmitMsg({ type: "error", message: "Please fill all required fields with valid information" });
      return;
    }
    ...
    try {
      ...
      for (const key in formData) {
        ...
        if (v !== undefined && v !== null) {
          formDataToSend.append(key, v as any);
        }
      }

      const res = await fetch(`${apiBase}/requestQuote/create`, {
        method: "POST",
        body: formDataToSend,
      });
      let data: any = null;
      ...
      if (res.ok && data?.status === "success") {
        setSubmitMsg({ type: "success", message: "Quote request submitted successfully!" });
        setIsLoading(false);
        setFormData(initialFormState);
        // router.push("/");
      } else {
        const msg = data?.message || "Submission failed";
        setSubmitMsg({ type: "error", message: msg });
        setIsLoading(false);
      }
    } catch (error) {
        ...
        <div className=" sm:w-10/12 w-full mx-auto">
          <div className="bg-[#f7f7f7] rounded-tl-[30px] sm:rounded-tl-[50px] overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-5 lg:p-6 space-y-3 sm:space-y-4 lg:space-y-5">
              ...
              <div className=" flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4">
                ...
                {isLoading ? (
                  ...
                  <button
                    ...
                    className={`px-6 sm:px-8 py-2.5 sm:py-3 text-sm sm:text-base font-bold rounded-lg text-white whitespace-nowrap transition-colors w-full sm:w-auto ${!validateForm()
              ...
              {submitMsg.type ? (
                <div className={`mt-3 text-sm ${submitMsg.type === "success" ? "text-green-600" : "text-red-600"}`}>
                  {submitMsg.message}
                </div>
              ) : null}
            </form>
