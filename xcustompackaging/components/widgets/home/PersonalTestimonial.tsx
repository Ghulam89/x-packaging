    ...
    return (
        <section className=" bg-white py-10 overflow-hidden relative">
            ...
            <div className="max-w-8xl mx-auto ">
                <div className='relative bg-cover bg-center bg-no-repeat' style={{ backgroundImage: `url(${ASSETS.misc.starsBg})` }}>
                    <div className='flex flex-col justify-between items-center pb-10'>
                        ...
                        <p className=' text-gray-800  mb-5'>Start creating your custom boxes effortlessly with our extensive library, featuring fully customizable layouts for promotional, mailer, and dispaly boxes,</p>
                        ...
                        {row1.length > 0 && (
                            <div className="flex overflow-hidden">
                                <div className="flex gap-6 animate-scroll-left">
                                    {[...row1, ...row1].map((t, idx) => (
                                        <TestimonialCard key={`row1-${(t as Testimonial)._id}-${idx}`} testimonial={t as Testimonial} />
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        {row2.length > 0 && (
                            <div className="flex overflow-hidden">
                                <div className="flex gap-6 animate-scroll-right">
                                    {[...row2, ...row2].map((t, idx) => (
                                        <TestimonialCard key={`row2-${(t as Testimonial)._id}-${idx}`} testimonial={t as Testimonial} />
                                    ))}
                                </div>
                            </div>
                        )}
