import { apiBase, getProductBySlug } from "@/lib/api";
...
export default async function ProductPage({
  ...
  const images = (product.images || []).map((img: any) =>
    img?.url ? `${apiBase}/${img.url.replace(/^\//, "")}` : ""
  ).filter(Boolean);
                  ...
                  <div>
                    ...
                    <div className=' w-4/12 mx-auto'>
                        ...
                        <div className=' w-full'>
                            <Image src={`${apiBase}/${(product as any).bannerImage}`} alt={product?.name || ''} width={500} height={500} className='  shadow-2xl w-full rounded-2xl' />
                        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {relatedProducts.map((item, index) => {
              const firstImage = item?.images?.[0]?.url
                ? `${apiBase}/${item.images[0].url.replace(/^\//, "")}`
                : undefined;
              return (
                <ProductCard
