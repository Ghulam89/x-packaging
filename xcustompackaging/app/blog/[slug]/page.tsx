import { getBlogBySlug, getBlogsAll, apiBase, apiGet } from "@/lib/api";
...
export default async function BlogPage({
  ...
  const blogImage = blog.image ? `${apiBase}/${blog.image.replace(/^\//, "")}` : "";
              ...
              <div className="mt-16 bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {blogProducts.map((product) => {
                    const img = product.images?.[0]?.url ? `${apiBase}/${product.images[0].url.replace(/^\//, "")}` : "";
                    return (
                      <ProductCard
                        key={product._id}
